...
function getCssStyleByDock(parent,miniIndex){var targetCss={};var screenWH=getBrowserScreenWH();var cpos=null;var bTop=0;var bLeft=0;var narrow=setting.minWinNarrow;var long=minWinData.long;if(setting.dockArea!=null){parent=$(setting.dockArea);}
if(parent!=null){cpos=parent.offset();bTop=parseInt(parent.css('borderTopWidth'));bLeft=parseInt(parent.css('borderLeftWidth'));long=parent.get(0)._minWinData.long;}
...
cornerClass=options.showRoundCorner?"ui-corner-top ":"";var headerHtml="<div class='window_header window_header_normal ui-widget-header "+cornerClass+"no-resizable "+options.headerClass+"'>"+"<img class='window_title_icon' src='"+options.icon+"' style='display:none;' onload='javascript:$.Window._iconOnLoad(this);'/>"+"<div class='window_title_text'>"+options.title+"</div>"+"<div class='window_function_bar'></div>"+"</div>";container.append(headerHtml);header=container.children("div.window_header");hideIcon();if(options.maximizable){header.dblclick(function(){if(maximized){restore();}else{maximize();}});}
...
hideIcon();var icon=header.children('.window_title_icon');icon.attr('src',options.icon);}
function showIcon(){var icon=header.children('.window_title_icon');icon.show();var txt=header.children('.window_title_text');txt.css('margin-left','0');textPanelWidthOffset=20;adjustHeaderTextPanelWidth();}
function hideIcon(){var icon=header.children('.window_title_icon');icon.hide();var txt=header.children('.window_title_text');txt.css('margin-left','-20px');textPanelWidthOffset=0;adjustHeaderTextPanelWidth();}
...
windowStorage=[];minWinData.storage=[];minWinData.long=setting.minWinLong;for(var i=0;i<parentCallers.length;i++){var mwdata=parentCallers[i].get(0)._minWinData;mwdata.storage=[];mwdata.long=setting.minWinLong;}},hideAll:function(){for(var i=0,len=windowStorage.length;i<len;i++){windowStorage[i].getContainer().hide();}},showAll:function(){for(var i=0,len=windowStorage.length;i<len;i++){windowStorage[i].getContainer().show();}},getAll:function(){return windowStorage;},getWindow:getWindow,getSelectedWindow:function(){for(var i=0,len=windowStorage.length;i<len;i++){var wnd=windowStorage[i];if(wnd.isSelected()){return wnd;}}},_iconOnLoad:function(element){log('iconOnLoad');var windowId=$(element).parent().parent().attr('id');var wnd=$.Window.getWindow(windowId);wnd.showIcon();}}})();$.window.getVersion=$.Window.getVersion;$.window.prepare=$.Window.prepare;$.window.closeAll=$.Window.closeAll;$.window.hideAll=$.Window.hideAll;$.window.showAll=$.Window.showAll;$.window.getAll=$.Window.getAll;$.window.getWindow=$.Window.getWindow;$.window.getSelectedWindow=$.Window.getSelectedWindow;